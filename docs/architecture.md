...
* ~~On document update, update all index containers using a single simple api `DocumentIndex`~~ — done,
  see `port.DocumentIndex` / `port.DocumentIndexSourceUpdate`, implemented by all index types
  including `internal/adapter/index/db_index_mango.go`.
* ~~Mango queries execute through a dedicated index rather than a bolted-on `FindDocs`~~ — done,
  `MangoIndex` implements `port.DocumentIndex` and is maintained like any other index; this remains
  an improvement over CouchDB, where Mango queries can only use Mango indices and `_all_docs`.
